...
const { extractDatesFromBody, readKeywordsFromDict } = require("../services/web-scraper-service")
const { extractDates,formatStringDate, convertDateInString, convertDateInString2, convertDateInString3 } = require("../untils/date")
...
const getSIGCOMMConferenceDetail = async (browser, conference) => {
    try {
        ...
        const importantDate = await page.$$eval(".ui-li-static.ui-body-inherit", (els) => {
            return els.map((el)=> {
                return el.innerText;
            })
        })
...
const getIARIAConferenceDetail = async (browser, conference) => {
    try {
        ...
        importantDate = importantDate[0].split('\n').filter(line => !/^\s*$/.test(line)).slice(0,8);
...
const getDOCENGConferenceDetail = async (browser, conference) => {
    try {
        ...
        await extractDatesFromBody(
            submissionDate_keywords,
            convertDateInString(importantDate[2].split('\t').slice(0,2).join(" ")),
            submissionDate,
            50
        );
        ...
        await extractDatesFromBody(
            submissionDate_keywords,
            convertDateInString2(importantDate[3].split('\t').slice(0,2).join(" ")),
            submissionDate,
            50
        );
        ...
        await extractDatesFromBody(
            notificationDate_keywords,
            convertDateInString(importantDate[4].split('\t').slice(0,2).join(" ")),
            notificationDate,
            50
        );
    ...
    try {
        ...
        if(conference._id == "6639c1b66e0c3f3fe99c564f") {
            ...
            if(dateFinder(getConfDate).length > 0) {
                const findDate = dateFinder(convertDateInString3(getConfDate))
                if(findDate == "") {
                    conferenceDate.push({
                ...
                getConfDate = await page.$$eval(".row.date-facets", (els) => {
                    return els.map((el) => {
                        return el.innerText
                    })
                })
            ...
            const importantDate = await page.$$eval(".table.table-hover.important-dates-in-sidebar", (els)=> {
                return els.map(el => {
                    return el.innerText
                })
            })
            ...
            let getCallForPaper = await page.$$eval("#Call-for-Papers", (els)=> {
                return els.map(el => {
                    return el.innerText
                })
            })
            ...
            if(getCallForPaper.length > 0) {
                callForPaper = getCallForPaper[0].replace("Call for Papers", "")
            } else {
                getCallForPaper = await page.$$eval("#Call-for-Research-Papers", (els)=> {
                    return els.map(el => {
                        return el.innerText
                    })
                })
