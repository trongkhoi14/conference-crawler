...
const datePatterns = [
  /\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{1,2}(?:-\d{1,2})?,?\s+\d{4}\b/g,
  /\b\d{1,2}(?:-\d{1,2})?\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?),?\s+\d{4}\b/g,
  /\b\d{4}-\d{2}-\d{2}\b/g,
    ...
    for (let i = 200; i < 201; i++) {
      ...
      if (compareLocations(expectedLocation, extractedLocation)) {
        ...
        if (compareLocations(expectedLocation, newExtractedLocation)) {
          ...
          if (isInDict(conferences[i].Links[0])) {
            console.log("True");
          } else {
