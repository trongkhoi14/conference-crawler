...
const getLocation = async (browser,link) => {
  ...
  if(result[0]){
    ...
    const resultFromHomepage= await isInDict(browser,trimUrl(link))
    if(resultFromHomepage[0]){
      return resultFromHomepage[1];
    }
  }
  return "null";
}
